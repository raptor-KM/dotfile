...
(custom-set-variables
 ...
 '(irony-additional-clang-options
   '("-pthread" "-std=c++11" "-m64" "-I/usr/local/root-6.22.02/cmake_build/include" "-L/usr/local/root-6.22.02/cmake_build/lib" "-lCore" "-lImt" "-lRIO" "-lNet" "-lHist" "-lGraf" "-lGraf3d" "-lGpad" "-lROOTVecOps" "-lTree" "-lTreePlayer" "-lRint" "-lPostscript" "-lMatrix" "-lPhysics" "-lMathCore" "-lThread" "-lMultiProc" "-lROOTDataFrame" "-pthread" "-lm" "-ldl" "-rdynamic" "-std=c++2a"))
 '(package-selected-packages
   '(ov mozc neotree open-junk-file textx-mode presentation helm-projectile projectile helm-selected helm-ag helm-smex which-key helm-img helm-swoop swoop volatile-highlights nyan-mode hlinum hiwin bind-chord diminish use-package yasnippet-classic-snippets yasnippet-snippets flycheck-irony flylisp company-irony-c-headers company-irony rainbow-mode undo-tree beacon sl helm-company helm elpy company-jedi jedi flycheck company quelpa snippet yasnippet-lean)))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(flycheck-error ((t (:background "DarkSeaGreen2" :underline (:color "Red1" :style wave))))))
...
;; (let ((default-directory (locate-user-emacs-file "/home/mizuno/.emacs.d/site-lisp")))
;;   (add-to-list 'load-path default-directory)
;;   (normal-top-level-add-subdirs-to-load-path))
;; (load (locate-user-emacs-file "/home/mizuno/.emacs.d/site-lisp/site-lisp-autoloads.el") t)

(defvar my/font-family "Migu 1M")
(defvar my/font-size
...
(require 'ov)
(defun my/font_tanuki ()
  "Minchoize current buffer."
  (interactive)
  (ov (point-min) (point-max) 'face '(:family "Tanuki Permanent Marker")))
(defun my/font_DejaVu ()
  "Minchoize current buffer."
  (interactive)
  (ov (point-min) (point-max) 'face '(:family "DejaVu Sans")))
...
(progn
  ...
  (bind-key* "C-c C-t"     'my/font_tanuki)
  (bind-key* "C-c C-d"     'my/font_DejaVu)
...
(use-package key-chord :defer t
  ...
  (key-chord-define-global "cv" 'ov-clear)
